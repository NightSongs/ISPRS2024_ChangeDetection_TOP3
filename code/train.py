...
def train(num_epochs, num_classes, optimizer, scheduler, loss_fn, train_loader, valid_loader, model, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        val_seg_iou = cal_val_iou(model, valid_loader, num_classes)
        if best_seg_iou <= val_seg_iou:
            best_seg_iou = val_seg_iou
            torch.save(model.state_dict(), save_path)
            logger.info(
                f'\n Epoch {epoch + 1}: Train Loss = {train_loss:.4f}, Val mIoU = {val_seg_iou:.4f}, lr = {lr:.6f}, model saved')
        else:
            logger.info(
                f'\n Epoch {epoch + 1}: Train Loss = {train_loss:.4f}, Val mIoU = {val_seg_iou:.4f}, lr = {lr:.6f}')
    return best_seg_iou
...
if __name__ == '__main__':
    ...
    # model_save_dir = "upernet_convnext_base_clip_drop0.5_droppath0.5_Convf_62e_w-o-aug"
    model_save_dir = "upernet_convnext_base_clip_Convf_62e_w-o-aug"
    ...
    model_save_path = f"code/checkpoints/{model_save_dir}/seg_model_1.pth"
